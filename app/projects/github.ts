...
export const getContributions = createServerFn().handler(
  ...
  > => {
    try {
      ...
      console.error(
        "[server/github] Failed to fetch contributions:",
        error,
      );
      return [];
    }
...
export const getRepos = createServerFn().handler(
  async (): Promise<Repository[] | undefined> => {
    try {
      ...
      return user.repositories.edges?.map(
        (edge) => edge?.node as Repository,
      );
    } catch (error) {
      console.error(
        "[server/github] Failed to fetch repositories:",
        error,
      );
      return [];
    }
